...
------------ | ------------- | ------------- | -------------
**DefaultTlsContainerRef** | Pointer to **NullableString** | 기본 TLS 인증서 참조 ID - &#x60;TERMINATED_HTTPS&#x60; 프로토콜을 사용하는 경우에만 적용 - &#x60;secret_ref&#x60; 값은 [List TLS certificates](/openapi/networking/lb/list-tls-certificates)에서 확인 | [optional] 
**SniContainerRefs** | Pointer to **[]string** | SNI 서버 이름에 따라 매칭되는 TLS 인증서 목록 | [optional] 
**TargetGroupId** | Pointer to **NullableString** | 연결할 기본 대상 그룹 ID - [List target groups](/openapi/networking/lb/list-target-groups)에서 확인 | [optional] 
**TlsMinVersion** | Pointer to [**NullableTLSVersion**](TLSVersion.md) | 리스너에서 허용할 최소 TLS 버전 (지정한 버전 이상의 TLS 프로토콜을 지원) ※ &#x60;TERMINATED_HTTPS&#x60; 프로토콜을 사용하는 경우에만 적용 | [optional] 
**TimeoutClientData** | Pointer to **NullableInt32** | 클라이언트 데이터 수신 타임아웃 시간 (ms) | [optional] 
**InsertHeaders** | Pointer to [**NullableInsertHeaderRequest**](InsertHeaderRequest.md) | 백엔드로 전달할 헤더 설정 | [optional] 
