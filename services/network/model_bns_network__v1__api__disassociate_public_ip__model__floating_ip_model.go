/*
Beyond Networking Service(BNS) Public IP/Security Group API

Public IP/Security Group API

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package network

import (
	"encoding/json"
	"fmt"
	"time"
)

// checks if the BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel{}

// BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel struct for BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel
type BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel struct {
	// 퍼블릭 IP의 고유 ID
	Id string `json:"id"`
	// 퍼블릭 IP의 현재 상태
	Status PublicIpStatus `json:"status"`
	// 퍼블릭 IP에 대한 설명
	Description string `json:"description"`
	// 퍼블릭 IP가 속한 프로젝트의 ID
	ProjectId string `json:"project_id"`
	// 외부에서 접근 가능한 퍼블릭 IP 주소 (IPv4 형식)
	PublicIp  string         `json:"public_ip"`
	PrivateIp NullableString `json:"private_ip,omitempty"`
	// 리소스가 생성된 시간 <br/> - ISO_8601 형식  <br/> - UTC 기준
	CreatedAt time.Time `json:"created_at"`
	// 리소스가 마지막으로 수정된 시간 <br/> - ISO_8601 형식  <br/> - UTC 기준
	UpdatedAt            time.Time `json:"updated_at"`
	AdditionalProperties map[string]interface{}
}

type _BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel

// NewBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel instantiates a new BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel(id string, status PublicIpStatus, description string, projectId string, publicIp string, createdAt time.Time, updatedAt time.Time) *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel {
	this := BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel{}
	this.Id = id
	this.Status = status
	this.Description = description
	this.ProjectId = projectId
	this.PublicIp = publicIp
	this.CreatedAt = createdAt
	this.UpdatedAt = updatedAt
	return &this
}

// NewBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModelWithDefaults instantiates a new BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModelWithDefaults() *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel {
	this := BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel{}
	return &this
}

// GetId returns the Id field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetId(v string) {
	o.Id = v
}

// GetStatus returns the Status field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetStatus() PublicIpStatus {
	if o == nil {
		var ret PublicIpStatus
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetStatusOk() (*PublicIpStatus, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetStatus(v PublicIpStatus) {
	o.Status = v
}

// GetDescription returns the Description field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetDescription() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Description
}

// GetDescriptionOk returns a tuple with the Description field value
// and a boolean to check if the value has been set.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Description, true
}

// SetDescription sets field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetDescription(v string) {
	o.Description = v
}

// GetProjectId returns the ProjectId field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetProjectId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ProjectId
}

// GetProjectIdOk returns a tuple with the ProjectId field value
// and a boolean to check if the value has been set.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetProjectIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ProjectId, true
}

// SetProjectId sets field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetProjectId(v string) {
	o.ProjectId = v
}

// GetPublicIp returns the PublicIp field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetPublicIp() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.PublicIp
}

// GetPublicIpOk returns a tuple with the PublicIp field value
// and a boolean to check if the value has been set.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetPublicIpOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PublicIp, true
}

// SetPublicIp sets field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetPublicIp(v string) {
	o.PublicIp = v
}

// GetPrivateIp returns the PrivateIp field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetPrivateIp() string {
	if o == nil || IsNil(o.PrivateIp.Get()) {
		var ret string
		return ret
	}
	return *o.PrivateIp.Get()
}

// GetPrivateIpOk returns a tuple with the PrivateIp field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetPrivateIpOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.PrivateIp.Get(), o.PrivateIp.IsSet()
}

// HasPrivateIp returns a boolean if a field has been set.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) HasPrivateIp() bool {
	if o != nil && o.PrivateIp.IsSet() {
		return true
	}

	return false
}

// SetPrivateIp gets a reference to the given NullableString and assigns it to the PrivateIp field.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetPrivateIp(v string) {
	o.PrivateIp.Set(&v)
}

// SetPrivateIpNil sets the value for PrivateIp to be an explicit nil
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetPrivateIpNil() {
	o.PrivateIp.Set(nil)
}

// UnsetPrivateIp ensures that no value is present for PrivateIp, not even an explicit nil
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) UnsetPrivateIp() {
	o.PrivateIp.Unset()
}

// GetCreatedAt returns the CreatedAt field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetCreatedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value
// and a boolean to check if the value has been set.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedAt, true
}

// SetCreatedAt sets field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetCreatedAt(v time.Time) {
	o.CreatedAt = v
}

// GetUpdatedAt returns the UpdatedAt field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetUpdatedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.UpdatedAt
}

// GetUpdatedAtOk returns a tuple with the UpdatedAt field value
// and a boolean to check if the value has been set.
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) GetUpdatedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.UpdatedAt, true
}

// SetUpdatedAt sets field value
func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) SetUpdatedAt(v time.Time) {
	o.UpdatedAt = v
}

func (o BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["status"] = o.Status
	toSerialize["description"] = o.Description
	toSerialize["project_id"] = o.ProjectId
	toSerialize["public_ip"] = o.PublicIp
	if o.PrivateIp.IsSet() {
		toSerialize["private_ip"] = o.PrivateIp.Get()
	}
	toSerialize["created_at"] = o.CreatedAt
	toSerialize["updated_at"] = o.UpdatedAt

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"status",
		"description",
		"project_id",
		"public_ip",
		"created_at",
		"updated_at",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel := _BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel{}

	err = json.Unmarshal(data, &varBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel)

	if err != nil {
		return err
	}

	*o = BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel(varBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "status")
		delete(additionalProperties, "description")
		delete(additionalProperties, "project_id")
		delete(additionalProperties, "public_ip")
		delete(additionalProperties, "private_ip")
		delete(additionalProperties, "created_at")
		delete(additionalProperties, "updated_at")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel struct {
	value *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel
	isSet bool
}

func (v NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) Get() *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel {
	return v.value
}

func (v *NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) Set(val *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) {
	v.value = val
	v.isSet = true
}

func (v NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) IsSet() bool {
	return v.isSet
}

func (v *NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel(val *BnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) *NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel {
	return &NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel{value: val, isSet: true}
}

func (v NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBnsNetworkV1ApiDisassociatePublicIpModelFloatingIpModel) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
