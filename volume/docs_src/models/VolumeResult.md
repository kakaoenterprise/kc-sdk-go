...
**AvailabilityZone** | Pointer to [**NullableAvailabilityZone**](AvailabilityZone.md) | 볼륨이 위치한 가용 영역 | [optional] 
...
**VolumeType** | Pointer to **NullableString** | 볼륨의 유형 | [optional] 
**SnapshotId** | Pointer to **NullableString** | 스냅샷의 고유 ID | [optional] 
**Metadata** | **map[string]string** | 볼륨에 설정된 사용자 정의 메타데이터(Key-Value 쌍) | 
**UserId** | Pointer to **NullableString** | 볼륨을 소유한 사용자 ID | [optional] 
**IsEncrypted** | Pointer to **NullableBool** | 암호화 여부 | [optional] 
**Attachments** | [**[]Attachment**](Attachment.md) | 볼륨의 인스턴스 연결 상태를 나타내는 Attachment 목록 - 새로 생성된 볼륨은 일반적으로 빈 배열 | 
